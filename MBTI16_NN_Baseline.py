...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("Embedding_Layer"):
            ...
            # self.x_ = tf.reshape(tf.nn.embedding_lookup(self.W_in_, self.input_w_), [-1, self.H*self.max_time_], name="x")
            self.x_ = tf.nn.embedding_lookup(self.W_in_, self.input_w_)
    
        # Construct RNN/LSTM cell and recurrent layer.
     
        with tf.name_scope("Recurrent_Layer"):
            #self.rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(self.H, forget_bias=0.0)
            self.cell_ = MakeFancyRNNCell(self.H, self.dropout_keep_prob_, self.num_layers)
            # self.initial_h_ = tf.reshape(self.cell_.zero_state(self.batch_size_, dtype=tf.float32),
        ...
        with tf.name_scope("Output_Layer"):
            ...
            self.logits_ = tf.add(matmul3d(self.output_, self.W_out_), self.b_out_, name="logits")

        # # Loss computation (true loss, for prediction)
        self.loss_ = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.target_y_,
                                                                                   logits=self.logits_,
                                                                                   name="loss"))
